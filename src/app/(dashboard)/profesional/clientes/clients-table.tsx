/**
 * @fileoverview Componente cliente para la tabla de clientes del profesional.
 * Recibe datos desde el server component y maneja búsqueda client-side.
 * Incluye acción: ver detalle del cliente.
 */
...
export function ClientsTable({ clients, metrics }: { clients: ClientRow[]; metrics: Metrics }) {
            ...
            >
                ...
                <div className="relative space-y-1">
                    ...
                    <p className="text-muted-foreground">
                        Historial de clientes que has atendido
                    </p>
                </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3" style={{ animation: 'fadeIn 0.4s ease-out 0.05s both' }}>
                <Card className="group relative overflow-hidden border-border/50 hover:shadow-lg hover:shadow-primary/5 transition-[box-shadow] duration-300">
                    ...
                    <CardContent className="p-5">
                        ...
                        <p className="text-3xl font-bold tracking-tight tabular-nums">{metrics.totalClients}</p>
                ...
                <Card className="group relative overflow-hidden border-border/50 hover:shadow-lg hover:shadow-blue-500/5 dark:hover:shadow-blue-500/10 transition-[box-shadow] duration-300">
                    ...
                    <CardContent className="p-5">
                        <div className="flex items-center justify-between mb-3">
                            <span className="text-sm font-medium text-muted-foreground">Visitas este mes</span>
                        ...
                        <p className="text-3xl font-bold tracking-tight tabular-nums">{metrics.visitsThisMonth}</p>
                        <p className="text-xs text-muted-foreground mt-1">citas este mes</p>
                    </CardContent>
                </Card>
                ...
                <Card className="group relative overflow-hidden border-border/50 hover:shadow-lg hover:shadow-amber-500/5 dark:hover:shadow-amber-500/10 transition-[box-shadow] duration-300">
                    ...
                    <CardContent className="p-5">
                        ...
                        <p className="text-3xl font-bold tracking-tight tabular-nums">{metrics.rating || '—'}</p>
                        <p className="text-xs text-muted-foreground mt-1">
                            {metrics.totalReviews > 0 ? `${metrics.totalReviews} reseñas` : 'sin reseñas aún'}
                        </p>
                    </CardContent>
                </Card>
            </div>
                                ...
                                <TableRow>
                                    <TableCell colSpan={6} className="text-center py-16">
                                        ...
                                        <p className="text-muted-foreground text-sm">
                                            {searchQuery ? 'No se encontraron clientes' : 'Aún no tienes clientes atendidos'}
                                        </p>
                                        <p className="text-xs text-muted-foreground/60 mt-1">
                                            {searchQuery
                                                ? 'Intenta con otros términos de búsqueda'
                                                : 'Tus clientes aparecerán aquí cuando tengas citas'}
                                        </p>
                                    </TableCell>
                                </TableRow>
                            ) : (
                                            ...
                                            <div className="inline-flex items-center gap-2">
                                                <div className="h-1.5 w-12 rounded-full bg-muted overflow-hidden">
                                                    <div
                                                        ...
                                                        style={{ width: `${Math.min((client.totalVisits / 20) * 100, 100)}%` }}
                                                    />
                                                </div>
